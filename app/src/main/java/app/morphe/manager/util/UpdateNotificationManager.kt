...
class UpdateNotificationManager(private val context: Context) {
    ...
    private fun postNotification(
        ...
        notificationAction: NotificationCompat.Action? = null,
    ) {
        val notification = NotificationCompat.Builder(context, CHANNEL_FCM_UPDATES)
            ...
            .setContentIntent(buildOpenAppIntent(openAction, notificationId))
            .setAutoCancel(true)
            .apply { notificationAction?.let { addAction(it) } }
            .build()
    ...
    private fun buildOpenAppIntent(
        action: String? = null,
        requestCode: Int = REQUEST_CODE_UPDATE_CHECK,
        triggerUpdateCheck: Boolean = true,
    ): PendingIntent {
        val intent = Intent(context, MainActivity::class.java).apply {
            flags = Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_CLEAR_TOP
            if (triggerUpdateCheck) putExtra(EXTRA_TRIGGER_UPDATE_CHECK, true)
            action?.let { putExtra(EXTRA_UPDATE_ACTION, it) }
    ...
    companion object {
        ...
        private const val NOTIFICATION_ID_AUTO_PATCH = 2010
        private const val NOTIFICATION_ID_AUTO_PATCH_BLOCKED = 2011
        private const val NOTIFICATION_ID_BATCH_RESULT = 2012
